...
function EntregablesFinales(){
  ...
  const getData=async()=> {
    (async () => {
      const result = await fetch(`https://localhost:7012/api/Entregable/ListEntregablesXIdCursoYIdTipoEntregableYIdAlumno?idCurso=${location.state.idCurso}&idTipoEntregable=4&idAlumno=${location.state.idAlumno}`);
  ...
  return(
      ...
      <h1 className='HEADER-TEXT1'>Entregables Finales</h1>
          ...
          <tbody >
            {filtrado.map(dato => (
              <tr key={dato.idEntregable}>
                ...
                <td>
                  <button class="btn btn-lg navbar-toggle" onClick={() =>navigate("entregableFinalSeleccionado",{state:{idVersion:dato.idVersionAntigua,idAlumno: location.state.idAlumno,tituloDoc:dato.nombre,notaVersion: dato.notaVersionMasReciente,
...
export default  EntregablesFinales;