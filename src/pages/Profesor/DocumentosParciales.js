...
function DocumentosParciales(){
  ...
  const getData=async()=> {
    (async () => {
      const result = await fetch(`http://34.195.33.246/api/Entregable/ListEntregablesXIdCursoYIdTipoEntregableYIdAlumno?idCurso=${location.state.idCurso}&idTipoEntregable=8&idAlumno=${location.state.idAlumno}`);
          ...
          <tbody >
            {filtrado.map(dato => (
              <tr key={dato.idEntregable}>
                ...
                <td>
                  <button class="btn btn-lg navbar-toggle" onClick={() =>navigate("documentoSeleccionado",{state:{idVersion:dato.idVersionAntigua,idAlumno: location.state.idAlumno,tituloDoc:dato.nombre,notaVersion: dato.notaVersionMasReciente,
...
export default  DocumentosParciales;