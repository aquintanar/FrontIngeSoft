...
function Exposiciones(){
  ...
  const getData=async()=> {
    (async () => {
      const result = await fetch(`https://localhost:7012/api/Entregable/ListEntregablesXIdCursoYIdTipoEntregableYIdAlumno?idCurso=${location.state.idCurso}&idTipoEntregable=2&idAlumno=${location.state.idAlumno}`);
          ...
          <tbody >
            {filtrado.map(dato => (
              <tr key={dato.idEntregable}>
                ...
                <td>
                  <button class="btn btn-lg navbar-toggle" onClick={() =>navigate("exposicionSeleccionada",{state:{idVersion:dato.idVersionAntigua,idAlumno: location.state.idAlumno,tituloDoc:dato.nombre,notaVersion: dato.notaVersionMasReciente,
