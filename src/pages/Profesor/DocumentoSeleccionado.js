...
const handleChange= (nombre,e)=>{
  ...
  const peticionA=async()=>{
    ...
    await axios(`http://34.195.33.246/api/Entregable/CalcularPuntajeEntregableXAlumno?idEntregable=${location.state.idEntregable}&idAlumno=${location.state.idAlumno}`)
    .catch(error =>{
      console.log(error.message);
    })
  }
...
const {
  ...
  prepareRow,
    ...
    return(
      ...
      <h1 >Entregable Final - { location.state.tituloDoc }</h1>
      <h2 >Alumno - {location.state.nombres} { location.state.apellidoPat } {location.state.apellidoMat}</h2>
...
export default  DocumentoSeleccionado;